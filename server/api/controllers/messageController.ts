...
export const addMessage = async (req: Request, res: Response): Promise<any> => {
  ...
  try {
    ...
    const emailSent = await sendEmail(name, email, message);  //sendEmail defined in email.ts

    if (emailSent) {
      return res.status(200).json({ success: true, message: 'Message sent successfully and email notified!' });
    } else {
